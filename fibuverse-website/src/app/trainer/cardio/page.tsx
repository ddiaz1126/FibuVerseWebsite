    ...
    <div className="flex h-full min-h-screen bg-gray-900 text-white">
      ...
      <div className="w-64 border-r border-gray-700 p-4 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto">
          {cardioItems
            .filter(item => item.type.toLowerCase() === activeTab.toLowerCase() && item.name.toLowerCase().includes(searchTerm.toLowerCase()))
            .map(item => (
              <div key={item.id} className="mb-1">
                <button
                  className={`w-full text-left px-3 py-2 rounded hover:bg-gray-700 transition ${
                    selectedCardio?.id === item.id ? "bg-gray-700" : ""
                  }`}
                  onClick={() => {
                    setSelectedCardio(item);
                    if (item.type === "program") setExpandedProgram(item.id === expandedProgram ? null : item.id);
                  }}
                >
                  {item.name}
                </button>
                ...
                {item.children && expandedProgram === item.id && (
                  <div className="ml-4 mt-1 space-y-1">
                    {item.children.map(run => (
                      <button
                        key={run.id}
                        className={`w-full text-left px-2 py-1 rounded hover:bg-gray-700 transition ${
                          selectedCardio?.id === run.id ? "bg-gray-700" : ""
                        }`}
                    ...
                    <button className="w-full text-left px-2 py-1 rounded bg-green-600 hover:bg-green-700 mt-1">
                      + Add Run
                    </button>
                  </div>
                )}
      ...
      <div className="flex-1 p-4 flex flex-col gap-4">
        {/* Top: Cardio metadata + Calendar */}
        <div className="flex gap-4">
        {/* Left: Cardio metadata */}
        <div className="w-96 bg-gray-800 p-4 rounded shadow flex flex-col gap-2">
            <label className="text-sm">Cardio Date</label>
            <input type="date" className="p-2 rounded bg-gray-700" />
            ...
            <textarea className="p-2 rounded bg-gray-700 flex-1" rows={4}></textarea>
        </div>
        ...
        <div className="flex-1 bg-gray-800 p-4 rounded shadow flex flex-col gap-2">
          <label className="text-sm">Run Name</label>
          <input type="text" className="p-2 rounded bg-gray-700" />
          <label className="text-sm">Main Metrics</label>
          <div className="flex gap-2">
            <input type="number" placeholder="Distance (km)" className="p-2 rounded bg-gray-700 flex-1" />
            <input type="number" placeholder="Duration (min)" className="p-2 rounded bg-gray-700 flex-1" />
            <input type="number" placeholder="Avg HR" className="p-2 rounded bg-gray-700 flex-1" />
          </div>
          {/* You can add more metric inputs here */}
        </div>
      </div>
    </div>
  );
