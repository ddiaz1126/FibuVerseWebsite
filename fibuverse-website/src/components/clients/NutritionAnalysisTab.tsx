...
interface NutritionAnalysisTabProps {
  clientId: number;
  nutritionMeta: any; // replace 'any' with proper typing if available
}

export default function NutritionAnalysisTab({ clientId, nutritionMeta }: NutritionAnalysisTabProps) {
    ...
    switch (activeMetric) {
      case "calories_per_week":
        return (
          <Bar
            data={{
              labels: nutritionMeta.calories_per_week.map((w: any) =>
                new Date(w.week).toLocaleDateString()
              ),
              datasets: [
                {
                  label: "Calories per Week",
                  data: nutritionMeta.calories_per_week.map((w: any) => w.total_calories),
      ...
      case "avg_macros_per_week":
        return (
          <Line
            data={{
              labels: nutritionMeta.avg_macros_per_week.map((w: any) =>
                new Date(w.week).toLocaleDateString()
              ),
              datasets: [
                {
                  label: "Protein (g)",
                  data: nutritionMeta.avg_macros_per_week.map((w: any) => w.avg_protein),
                  borderColor: colors[0],
                  backgroundColor: colors[0],
                },
                {
                  label: "Carbs (g)",
                  data: nutritionMeta.avg_macros_per_week.map((w: any) => w.avg_carbs),
                  borderColor: colors[1],
                  backgroundColor: colors[1],
                },
                {
                  label: "Fat (g)",
                  data: nutritionMeta.avg_macros_per_week.map((w: any) => w.avg_fat),
